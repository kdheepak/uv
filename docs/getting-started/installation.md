...
=== "macOS and Linux"
    ...
    ```console
    $ UV_INSTALL_DIR="/custom/path" bash -c 'curl -LsSf https://astral.sh/uv/install.sh | sh'
    ```

=== "Windows"

    ```powershell
    $env:UV_INSTALL_DIR = "C:\Custom\Path" powershell -c "irm https://astral.sh/uv/install.ps1 | iex"
    ```
